...
final class BoardSnapshotService {
    ...
    BoardSnapshot read(UUID boardId, UUID ownerId) {
        var board = jdbc.query("""
                ...
                """, result -> result.next() ? new BoardRow(
                        ...
                        result.getBoolean("background_present"),
                        SignatureInkColor.fromDatabase(result.getString("signature_ink_color"))) : null,
                boardId, ownerId);
    ...
    private BoardSnapshot snapshot(UUID boardId, BoardRow board) {
        ...
        return new BoardSnapshot(boardId, board.width(), board.height(), board.backgroundPresent(),
                board.signatureInkColor(), slots);
    }
    ...
    private BoardSnapshot.Slot slot(UUID boardId, ResultSet result) throws SQLException {
        ...
        return new BoardSnapshot.Slot(id, result.getBigDecimal("x"), result.getBigDecimal("y"),
                ...
                signature, signature == null ? live.signature() : null, live.draftEpoch(), live.revision());
    }
    ...
    private record BoardRow(
            int width, int height, boolean backgroundPresent, SignatureInkColor signatureInkColor) {}
}

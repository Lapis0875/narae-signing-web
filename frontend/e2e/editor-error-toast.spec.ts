...
const evidenceDir =
  process.env.EDITOR_ERROR_TOAST_EVIDENCE_DIR ??
  path.resolve("../.omo/evidence/editor-error-toast");
...
function malformedBoard(status: "생성 중" | "설정 중") {
  return {
    canvasHeight: 1080,
    canvasWidth: 1920,
    createdAt: now,
...
async function captureToast(page: Page, state: string) {
  ...
  for (const [width, height] of [
    ...
    await page.screenshot({
      fullPage: false,
      path: path.join(evidenceDir, `${state}-${width}.png`),
    });
  }
}

async function stubEditorApi(
  page: Page,
  boardResponse: ReturnType<typeof board> | ReturnType<typeof malformedBoard>,
) {
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    if (pathname === "/api/v1/auth/session") {
      return route.fulfill({
        json: { authenticated: true, expiresAt: "2026-08-21T01:00:00.000Z" },
      });
    }
    if (
      ...
      return route.fulfill({ json: boardResponse });
    ...
    if (pathname === `/api/v1/admin/boards/${boardId}/share`)
      return route.fulfill({ json: { shareToken: "safe-share", version: 1 } });
...
test("shows a lower-right diagnostic toast for an incompatible editor response", async ({
  context,
  page,
}) => {
  // Given
  await context.addCookies([
    { name: "XSRF-TOKEN", url: "http://127.0.0.1:4173", value: "test-csrf" },
  ]);
  await stubEditorApi(page, malformedBoard("생성 중"));
  ...
  await expect(toast(page)).toContainText("서버 응답을 확인할 수 없습니다.");
  await expect(toast(page)).toContainText("HTTP 200");
  await expect(toast(page)).toContainText("MALFORMED_RESPONSE");
  await expect(toast(page)).not.toContainText("status");
  await captureToast(page, "editor-contract-error");
});

test("shows a lower-right diagnostic toast when roster addition is rejected", async ({
  context,
  page,
}) => {
  // Given
  ...
  await stubEditorApi(page, board("설정 중"));
  await page.goto(`/boards/${boardId}/edit`);

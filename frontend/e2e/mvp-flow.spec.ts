...
function requiredEnvironment(name: string): string {
  ...
  if (value === undefined || value.length === 0)
    throw new Task30ConfigurationError(name);
  return value;
}
...
class MvpApi {
  ...
  async handle(route: Route): Promise<void> {
    ...
    if (key === "GET /api/v1/admin/boards") {
      return route.fulfill({
        json: this.boardCreated && !this.deleted ? [this.board()] : [],
      });
    }
    ...
    if (key === `PATCH /api/v1/admin/boards/${boardId}/slots/${slotId}`) {
      ...
      expect(bounds).toEqual({ height: 0.18, width: 0.24, x: 0, y: 0 });
      this.roster = [
        {
          ...this.roster[0],
          slot: {
    ...
    if (key === `GET /api/v1/admin/boards/${boardId}/events`) {
      ...
      return route.fulfill({
        body: "event: signature-submitted\ndata: {}\n\n",
    ...
    if (key === `GET /api/v1/public/links/${shareToken}`) {
      return route.fulfill({
        json: {
          state: this.boardStatus === "마감/보관" ? "CLOSED" : "OPEN",
          title: "통합 검증 행사",
        },
      });
    }
    ...
    if (key === "GET /api/v1/public/signing-session") {
      const identified = this.calls.includes(
        `POST /api/v1/public/links/${shareToken}/identify`,
      );
      if (!identified)
        return route.fulfill({ json: { code: "UNAUTHORIZED" }, status: 401 });
      return route.fulfill({
        json: {
          signatureAspectRatio: 16 / 9,
          state: this.submitted ? "SUBMITTED" : "READY",
        },
      });
    }
    if (key === "PUT /api/v1/public/signing-session/draft") {
      ...
      this.draft = true;
      return route.fulfill({ json: { draftEpoch: 0, revision: 1 } });
    }
    if (key === "POST /api/v1/public/signing-session/draft/delta") {
      const delta = z
        .strictObject({
          clientSequence: z.number().int().nonnegative(),
          draftEpoch: z.number().int().nonnegative(),
          operation: z.enum(["append", "begin", "end"]),
          points: z.array(z.strictObject({ x: z.number(), y: z.number() })),
      ...
      this.draft = true;
      return route.fulfill({
        json: { draftEpoch: delta.draftEpoch, revision: delta.revision + 1 },
      });
    }
...
async function isolate(
  ...
  await context.addCookies([
    { name: "XSRF-TOKEN", value: "synthetic-csrf", url: origin },
  ]);
...
test("manager and signer complete one integrated synthetic event", async ({
  baseURL,
  browser,
}, testInfo) => {
  test.skip(
    process.env.TASK30_MVP_MODE === "real",
    "mock contract is distinct from the authorized live-stack run",
  );
  ...
  const signerContext = await browser.newContext({ baseURL: origin });
  await isolate(managerContext, api, origin);
  await isolate(signerContext, api, origin);
  ...
  await manager.getByLabel("PNG 또는 JPEG").setInputFiles({
    buffer: png,
    mimeType: "image/png",
    name: "synthetic.png",
  });
  ...
  await expect(
    manager.getByText(`/sign/${shareToken}`, { exact: false }),
  ).toBeVisible();
  ...
  expect(api.calls).toEqual(
    expect.arrayContaining([
      ...
      "POST /api/v1/public/signing-session/draft/delta",
  ...
  await testInfo.attach("mvp-api-calls", {
    body: JSON.stringify(api.calls, null, 2),
    contentType: "application/json",
  });
  await testInfo.attach("mvp-final-dom", {
...
test("authorized live stack completes one integrated synthetic event", async ({
  browser,
}) => {
  test.skip(
    process.env.TASK30_MVP_MODE !== "real",
    "requires the separately authorized isolated Compose stack",
  );
  ...
  await manager.getByRole("button", { name: "로그인" }).click();
  await expect(
    manager.getByRole("heading", { level: 1, name: "보드 목록" }),
  ).toBeVisible();
  ...
  await fullView.goto(
    new URL(`/boards/${boardIdentifier}/full`, baseUrl).toString(),
  );
  await expect(fullView.getByTestId("submitted-signature")).toHaveCount(0);
  ...
  if (replacementBox === null)
    throw new Task30ConfigurationError("replacement signer canvas");
  await replacement.mouse.move(replacementBox.x + 40, replacementBox.y + 40);
  await replacement.mouse.down();
  await replacement.mouse.move(replacementBox.x + 140, replacementBox.y + 90, {
    steps: 4,
  });

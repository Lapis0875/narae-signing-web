...
const evidenceDir =
  ...
  path.resolve("../.omo/evidence/public-display-live-ink");
const viewports = [
...
async function metrics(page: Page, scenarioId: string): Promise<Runtime> {
  ...
  if (viewport === null)
    throw new Error("Visual matrix viewport is unavailable");
  const board = page.getByTestId("full-view-canvas");
  const boardBounds =
    (await board.count()) === 0
      ? undefined
      : ((await board.boundingBox()) ?? undefined);
...
async function routePublic(
  ...
  await context.route(
    ...
    (route: Route) => {
      ...
      if (suffix === "title")
        return route.fulfill({ json: { title: "행사장 공개 화면" } });
...
async function capturePublic(
  page: Page,
  state: "progressive" | "denial" | "old-replaced",
  label: string,
): Promise<Runtime> {
  ...
  if (state === "denial") {
    ...
    if (state === "progressive") {
      ...
      await expect(
        page.locator(`[data-slot-id="${slotId}"] canvas`),
      ).toBeVisible();
    } else {
      await page.evaluate(() =>
        window.dispatchEvent(
  ...
  if (result.boardBounds !== undefined) {
    expect(
      result.boardBounds.y + result.boardBounds.height,
    ).toBeLessThanOrEqual(result.viewport.height);
    expect(
      Math.abs(result.boardBounds.width / result.boardBounds.height - 4 / 3),
    ).toBeLessThan(0.01);
  }
  const filename =
    state === "old-replaced"
...
async function routeAdmin(
  ...
  await context.route("**/api/v1/**", (route: Route) => {
    ...
    if (pathname === "/api/v1/auth/session")
      return route.fulfill({
        json: { authenticated: true, expiresAt: "2026-09-08T00:00:00.000Z" },
      });
...
async function captureAdmin(page: Page, label: string): Promise<Runtime> {
  ...
  await expect(page.getByRole("dialog", { name: "확인" })).toContainText(
    "현재 행사장 화면의 표시 연결을 종료합니다",
  );
  await settle(page);
  const result = await metrics(page, `admin-share-panel-confirm-${label}`);
  expect(result.horizontalOverflow).toBe(false);
  await page.screenshot({
    path: path.join(evidenceDir, `visual-final-admin-confirm-${label}.png`),
  });
  return result;
}

test("captures the public display final visual matrix", async ({
  baseURL,
  context,
}) => {
  ...
  await routeAdmin(context, new URL(baseURL ?? "http://127.0.0.1:4173").origin);
  ...
  await writeFile(
    path.join(evidenceDir, "visual-final-runtime.json"),
    `${JSON.stringify(runtime, null, 2)}\n`,
  );
  await writeFile(
    path.join(evidenceDir, "visual-final-manifest.json"),
    `${JSON.stringify(
      {

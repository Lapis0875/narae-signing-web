...
const evidenceDir =
  ...
  process.env.TASK22_EVIDENCE_DIR ??
  path.resolve("../.omo/evidence/task22-admin-board-ui");
...
async function collectTopElementEvidence(
  ...
  const headingBounds = expectInViewport(
    "h1",
  ...
  if (requirements.navigation) {
    await expect(header.locator('nav[aria-label="관리자 세션"]')).toHaveCount(
      1,
    );
  }
  expect(await page.evaluate(() => window.scrollY)).toBe(0);
  expect(
    await page.evaluate(() => window.visualViewport?.pageTop ?? window.scrollY),
  ).toBe(0);

  return {
    header: headerBounds,
    heading: headingBounds,
    navigation: navigationBounds,
  };
}

async function collectStickyHeaderEvidence(
  page: Page,
  viewport: ViewportSize,
): Promise<TopElementEvidence> {
  const header = page.locator(".app-header").first();
  ...
  const headerBounds = expectInViewport(
    "sticky header",
    await header.boundingBox(),
    viewport,
  );
  expect(headerBounds.y).toBe(0);
  const navigationBounds = expectInViewport(
    "sticky navigation",
...
test("creates a board and keeps the server roster snapshot through import errors", async ({
  baseURL,
  context,
  page,
}) => {
  ...
  page.on("console", (message) => {
    if (message.type() === "error") {
      if (
        /^Failed to load resource: the server responded with a status of (400 \(Bad Request\)|409 \(Conflict\))$/u.test(
          message.text(),
        )
      ) {
        expectedNetworkErrors.push(message.text());
      } else if (
        /^api_request_failed \{code: (?:UNKNOWN|MALFORMED_RESPONSE|CONFLICT|SERVICE_UNAVAILABLE), method: (?:GET|POST), requestId: null, route: \/api\/v1\/(?:admin\/boards|admin\/boards\/:id\/roster\/import|public\/(?:links\/:token|signing-session)), status: (?:0|200|400|409)\}$/u.test(
          message.text(),
        )
      ) {
  ...
  await context.addCookies([
    { name: "XSRF-TOKEN", value: "csrf-test", url: origin },
  ]);
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (pathname === "/api/v1/admin/boards" && request.method() === "POST") {
      ...
      boards = [board];
      await route.fulfill({
        json: { board, shareToken: "never-render-this-token" },
      });
      return;
    }
    if (pathname === `/api/v1/admin/boards/${boardId}`) {
      await route.fulfill({
        json: request.headers().referer?.endsWith(`/boards/${boardId}/full`)
          ? { status: "ready" }
          : board,
      });
      return;
    ...
    if (pathname === "/api/v1/public/links/safe-share") {
      await route.fulfill({
        json: { state: "OPEN", title: "가을 서명 발표회" },
      });
      return;
    }
    if (pathname === "/api/v1/public/links/safe-share/identify") {
      expect(request.postDataJSON()).toEqual({
        job: "진행",
        name: "하늘",
        organization: "나래 테스트",
      });
      await route.fulfill({ json: { identified: true } });
      return;
    }
    ...
    if (
      pathname === "/api/v1/public/signing-session/draft" &&
      request.method() === "PUT"
    ) {
      expect(request.postDataJSON()).toMatchObject({ version: 1 });
      await route.fulfill({ json: { draftEpoch: 0, revision: 1 } });
      return;
    }
    if (
  ...
  const pastedRows = Array.from(
    { length: 25 },
    (_, index) => `나래초\t교사\t서명자${String(index + 1).padStart(2, "0")}`,
  ).join("\n");
  ...
  await expect(page.locator('input[name="name"]').last()).toHaveValue(
    "서명자25",
  );
  ...
  await collectTopElementEvidence(page, initialViewport, {
    header: true,
    navigation: true,
  });
  await page.screenshot({
    path: path.join(evidenceDir, "task-22-admin-board-ui.png"),
    fullPage: true,
  });
  ...
  // Then
  await expect(
    page.getByRole("alert", { name: "명단 유지 안내" }),
  ).toContainText("서버 명단은");
  ...
  await expect(serverErrors.locator("li").nth(0)).toHaveAttribute(
    "data-row",
    "2",
  );
  await expect(serverErrors.locator("li").nth(1)).toHaveAttribute(
    "data-row",
    "3",
  );
  await collectTopElementEvidence(page, initialViewport, {
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "task-22-admin-board-ui-error.png"),
    fullPage: true,
  });
  ...
  const capture = async ({
    ...
    const isUnsupportedPhone =
      state === "public-signer-handoff" && viewport.width === 375;
    ...
    const focusedControl =
      mode.kind === "focused"
        ? expectInViewport(
            "focused control",
            await mode.control.boundingBox(),
            viewport,
          )
        : null;
    ...
    const pageTop =
      scrollY === 0 &&
      (await page.evaluate(
        () => (window.visualViewport?.pageTop ?? window.scrollY) === 0,
      ));
    ...
    const hash = createHash("sha256")
      ...
      .digest("hex");
    expect(
      captureHashes.has(hash),
      `${file} duplicates ${captureHashes.get(hash) ?? "another capture"}`,
    ).toBe(false);
  ...
  for (const viewport of viewports) {
    ...
    const logoutButton = page
      .getByRole("navigation", { name: "관리자 세션" })
      .getByRole("button", {
        name: "로그아웃",
      });
    ...
    await expect(
      page.getByRole("button", { name: "보드 만들기" }),
    ).toBeVisible();
    ...
    await expect(pendingCreateButton).toHaveCSS(
      "background-color",
      "rgb(240, 243, 248)",
    );
    ...
    await pendingCreateRoute.fulfill({
      json: { code: "BOARD_TITLE_DUPLICATE" },
      status: 409,
    });
    await expect(page.getByRole("alert")).toContainText(
      "보드를 만들지 못했습니다",
    ...
    await page.getByRole("button", { name: "파일 가져오기" }).click();
    await expect(
      page.getByRole("alert", { name: "서버 입력 오류" }),
    ).toContainText("2행: 같은 명단이 중복되었습니다.");
    await expect(page.locator('input[name="name"]').last()).toHaveValue(
      "서명자25",
    );
    ...
    expect(
      await keptPhrase.evaluate((element) => element.getClientRects().length),
    ).toBe(1);
    await capture({
      route: `/boards/${boardId}/edit`,
      state: "roster-server-error",
      viewport,
    });
    ...
    await expect(
      page.getByRole("heading", { level: 2, name: "명단 직접 추가" }),
    ).toHaveClass("board-section-title");
    await expect(
      page.getByRole("heading", { level: 2, name: "명단 가져오기" }),
    ).toHaveClass("board-section-title");
    ...
    await expect(dialog.getByRole("button", { name: "취소" })).toBeFocused();
    await expect(dialog.getByRole("button", { name: "삭제" })).toHaveCSS(
      "background-color",
      "rgb(11, 53, 88)",
    );
    ...
    const layoutViewportWidth = await page.evaluate(
    ...
    await expect(
      page.getByRole("heading", { name: "보드 전체보기" }),
    ).toBeVisible();
    await expect(page.getByTestId("full-view-canvas")).toBeAttached();
    await capture({
      route: `/boards/${boardId}/full`,
      state: "full-view-handoff",
      viewport,
    });
    ...
    if (viewport.width === 375) {
      ...
      const signingSessionResponse = page.waitForResponse((response) => {
      ...
      await page.getByLabel("소속사 (선택)").fill("나래 테스트");
      await page.getByLabel("직책 (선택)").fill("진행");
    ...
    await capture({
      route: "/sign/safe-share",
      state: "public-signer-handoff",
      viewport,
    });
  }

  await writeFile(
    path.join(evidenceDir, "matrix-manifest.json"),
    `${JSON.stringify({ captures: matrix, expectedApiDiagnostics, expectedBrowserNoise }, null, 2)}\n`,
  );

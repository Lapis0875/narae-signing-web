...
const evidenceDir =
  process.env.TASK28_EVIDENCE_DIR ??
  process.env.TASK27_EVIDENCE_DIR ??
...
test("closed board exposes explicit final PNG retry in the production editor toolbar", async ({
  context,
  page,
}) => {
  ...
  await page.route("**/api/v1/**", async (route: Route) => {
    ...
    if (pathname === `/api/v1/admin/boards/${boardId}/share`) {
      return route.fulfill({
        json: { shareToken: "synthetic-share", version: 1 },
      });
    }
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: "완료된 서명 보드" }),
  ).toBeVisible();
  ...
  expect(
    await page
      .locator(".editor-canvas-background")
      .evaluate(
        (image) =>
  ...
  for (const [width, height] of [
    ...
    await page
      .getByRole("heading", { level: 1, name: "완료된 서명 보드" })
      .click();
    await settle(page);
    ...
    if (bounds !== null && deleteBounds !== null) {
      ...
      const separated =
        bounds.x + bounds.width <= deleteBounds.x ||
    ...
    await writeFile(
      path.join(evidenceDir, `actual-editor-rest-${width}.json`),
      `${JSON.stringify(
        {
          actionBounds: bounds,
          deleteActionBounds: deleteBounds,
          eventRefetchRequests: {
            backgroundRequests,
            boardRequests,
            eventRequests,
            rosterRequests,
    ...
    await expect(
      confirmation.getByRole("button", { name: "영구 삭제" }),
    ).toBeVisible();
    await page.screenshot({
      fullPage: false,
      path: path.join(
        evidenceDir,
        `actual-editor-delete-confirmation-${width}.png`,
      ),
    });
  ...
  await page
    .getByRole("heading", { level: 2, name: "배경" })
    .scrollIntoViewIfNeeded();
  await page.screenshot({
    fullPage: false,
    path: path.join(evidenceDir, "actual-editor-background-375.png"),
  });
  await toolbar.scrollIntoViewIfNeeded();
  ...
  await page.screenshot({
    fullPage: false,
    path: path.join(evidenceDir, "actual-editor-focus-375.png"),
  });
  await action.click();
  ...
  if (errorGroupBounds !== null && toolbarBounds !== null) {
    ...
    expect(errorGroupBounds.x + errorGroupBounds.width).toBeLessThanOrEqual(
      toolbarBounds.x + toolbarBounds.width,
    );
  }
  if (
    alertBounds !== null &&
    errorReopenBounds !== null &&
    errorRetryBounds !== null
  ) {
    expect(alertBounds.y).toBeGreaterThanOrEqual(
      Math.max(
        errorReopenBounds.y + errorReopenBounds.height,
        errorRetryBounds.y + errorRetryBounds.height,
      ),
    );
  }
  ...
  await writeFile(
    path.join(evidenceDir, "actual-editor-error-375.json"),
  ...
  await page.screenshot({
    fullPage: false,
    path: path.join(evidenceDir, "actual-editor-error-375.png"),
  });

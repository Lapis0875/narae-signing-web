...
test("captures every responsive visual-foundation state", async ({ page }, testInfo) => {
  // Given
  ...
  await settleVisualFrame(page);
  await page.screenshot({ path: testInfo.outputPath("loading-1280x800.png") });
  releaseLoading?.();
  apiMode = "ready";
  await expect(page.getByLabel("이메일")).toBeVisible();
  ...
  await settleVisualFrame(page);
  await page.screenshot({ path: testInfo.outputPath("forbidden-1280x800.png") });
  await page.evaluate(() => window.localStorage.removeItem("visual-api-mode"));
  ...
  await expect(page.getByRole("heading", { level: 1, name: "서명하기" })).toBeVisible();
  await settleVisualFrame(page);
  await page.screenshot({ path: testInfo.outputPath("route-panel-1280x800.png") });
  ...
  await settleVisualFrame(page);
  await page.screenshot({ path: testInfo.outputPath("unsupported-375x812.png") });

...
for (const viewport of viewports) {
  test(`full view uses authoritative snapshots at ${viewport.label}`, async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("full-view-canvas")).toContainText(
      "전체보기를 불러오는 중입니다.",
    );
    ...
    if (initialSnapshot === null)
      throw new Error(
        "Expected the initial snapshot request to remain pending",
      );
    await initialSnapshot.fulfill({
      contentType: "application/json",
      body: snapshotBody(),
    });
    ...
    await expect(
      page.getByRole("img", { name: "서명 보드 전체보기" }),
    ).toHaveCount(0);
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-loading.png`,
    });
    ...
    if (initialBackground === null)
      throw new Error(
        "Expected the initial background request to remain pending",
      );
    ...
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-draft.png`,
    });
    ...
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-draft-cleared.png`,
    });
    ...
    await expect(
      page.locator(".slot-overlay, input, textarea, select"),
    ).toHaveCount(0);
    await expect(
    ...
    await page
      .getByRole("heading", { name: "보드 전체보기" })
      .evaluate((heading) => heading.scrollIntoView({ block: "start" }));
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-normal.png`,
    });
    if (viewport.label === "1280x800") {
      await page.screenshot({
        path: "../.omo/evidence/task-25-realtime-fullview/task-25-realtime-fullview.png",
        fullPage: true,
      });
    }
    ...
    await page.evaluate(() =>
      window.dispatchEvent(
        new CustomEvent("qa-sse-event", {
          detail: { id: 12, type: "board-updated" },
        }),
      ),
    );
    await expect(page.getByTestId("full-view-canvas")).toContainText(
    ...
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-query-error.png`,
    });
    ...
    await page
      ...
      .evaluate((heading) => heading.scrollIntoView({ block: "start" }));
    await page.screenshot({
      path: `../.omo/evidence/task-25-realtime-fullview/browser-matrix/${viewport.label}-recovery.png`,
    });
    if (viewport.label === "1280x800") {
      await page.screenshot({
        path: "../.omo/evidence/task-25-realtime-fullview/task-25-realtime-fullview-error.png",
        fullPage: true,
      });
    }
    expect(browserErrors).toEqual([]);

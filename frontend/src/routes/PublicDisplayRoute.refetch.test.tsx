...
describe("public display refetch retention", () => {
  it.each(["display-claim", "display-title"] as const)(
    "keeps the good display connected when a later %s refetch returns 503",
    async (queryName) => {
      // Given
      ...
      if (source === null)
        throw new Error("Expected a public display EventSource");
      ...
      await act(async () => {
        await client.refetchQueries({
          queryKey: [...queryKeyPrefix, queryName],
        });
      });
      ...
      expect(
        screen.getByRole("heading", { name: "행사장 공개 화면" }),
      ).toBeVisible();
  ...
  it.each([
    {
      body: { code: "PUBLIC_DISPLAY_UNAVAILABLE" },
      message: "행사장 화면을 불러오지 못했습니다.",
      status: 404,
    },
    {
      body: {
    ...
    "terminates a later claim refetch with HTTP $status",
    async ({ body, message, status }) => {
      // Given
      vi.stubGlobal("EventSource", TestEventSource);
      vi.stubGlobal(
        "ResizeObserver",
      ...
      await act(async () => {
        await client.refetchQueries({
          queryKey: [...queryKeyPrefix, "display-claim"],
        });
      });

...
import { mergePublicSnapshot, parsePublicDraftEvent, reconcilePublicDraft, reconnectDelay } from "./realtime.ts"
...
function draftEvent(revision: number, operation: "begin" | "append", x: number) {
  return parsePublicDraftEvent(JSON.stringify({
    draftEpoch: 1,
    operation,
    points: [{ x, y: x }],
    revision,
...
describe("public incremental realtime", () => {
  it("applies 1,2, ignores duplicate 2, and recovers on gap 4 without changing the other slot", () => {
    ...
    const second = reconcilePublicDraft(first.snapshot, draftEvent(2, "append", 20))
    if (second.kind !== "applied") throw new Error("revision 2 must apply")

    expect(second.snapshot.slots[0]?.draftSignature?.strokes[0]?.points).toEqual([{ x: 10, y: 10 }, { x: 20, y: 20 }])
    expect(second.snapshot.slots[1]).toBe(initial.slots[1])
    expect(reconcilePublicDraft(second.snapshot, draftEvent(2, "append", 20))).toEqual({ kind: "ignored" })
    expect(reconcilePublicDraft(second.snapshot, draftEvent(4, "append", 40))).toEqual({ kind: "recover" })
  })

  it("rejects malformed events at the boundary", () => {
    expect(() => parsePublicDraftEvent("{\"operation\":\"append\",\"points\":\"bad\"}")).toThrow()
  })

  it("applies only the next reset epoch and converges clear, cancel, and full-reset payloads", () => {
    const initial = snapshot()
    const fullReset = parsePublicDraftEvent(JSON.stringify({
      draftEpoch: 2,
    ...
    const clear = parsePublicDraftEvent(JSON.stringify({
      draftEpoch: 3,
      operation: "clear",
      points: [],
      revision: 0,
    ...
    const cancel = parsePublicDraftEvent(JSON.stringify({
      draftEpoch: 4,
      operation: "clear",
      points: [],
      revision: 0,
    ...
    if (resetResult.kind !== "applied") throw new Error("full reset must apply")
    ...
    expect(resetResult.snapshot.slots[0]?.draftSignature).toEqual({ strokes: [{ points: [{ x: 30, y: 40 }] }], version: 1 })
    expect(clearResult.snapshot.slots[0]?.draftSignature).toEqual({ strokes: [], version: 1 })
    expect(cancelResult.kind === "applied" ? cancelResult.snapshot.slots[0]?.draftSignature : undefined).toBeNull()
    expect(reconcilePublicDraft(initial, { ...fullReset, draftEpoch: 3 })).toEqual({ kind: "recover" })
  })
  ...
  it("ignores stale snapshots and accepts equal snapshots including submitted geometry", () => {
    ...
    const submittedPayload = { strokes: [{ points: [{ x: 10, y: 10 }] }], version: 1 as const }
    equal.slots[0] = { ...equalSlot, draftSignature: null, signature: submittedPayload }
    ...
    expect(afterStale.slots[0]?.draftSignature).toEqual(applied.snapshot.slots[0]?.draftSignature)

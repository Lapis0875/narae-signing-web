...
class ControlledEventSource extends EventTarget {
  ...
  emit(type: string) { this.dispatchEvent(new MessageEvent(type, { data: "{}" })) }
}

function snapshot(signed: boolean): Response {
  return Response.json({
    backgroundPresent: false, boardId, canvasHeight: 600, canvasWidth: 800,
    slots: [{
      background: "transparent", draftEpoch: 1,
      draftSignature: signed ? { strokes: [{ points: [{ x: 100, y: 200 }] }], version: 1 } : null,
      height: 0.2, id: slotId, revision: signed ? 1 : 0, signature: null,
      width: 0.3, x: 0.1, y: 0.2,
    }],
  })
}
...
it("renders incoming draft while a background update response remains pending", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    ...
    return backgroundRequests <= 2 ? new Response(null, { status: 204 }) : backgroundResponse.promise
  })
  const { source } = await display()
  await act(async () => { source.emit("background-updated") })
  expect(backgroundRequests).toBe(3)

  try {
    // When
    signed = true
    await act(async () => { source.emit("signature-draft") })

    // Then
    expect(await screen.findByTestId("submitted-signature")).toBeVisible()
    expect(backgroundRequests).toBe(3)
  } finally {
    await act(async () => { backgroundResponse.resolve(new Response(null, { status: 204 })) })
  }
})

it.each(["signature-draft", "signature-draft-cleared", "signature-submitted", "signature-reset"])(
  "refreshes visible snapshot without downloading unchanged background for %s", async (eventType) => {
    // Given
    let signed = false
    let backgroundRequests = 0
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    ...
    // When
    signed = true
    await act(async () => { source.emit(eventType) })
...
it("keeps one final snapshot follow-up for a burst while the current snapshot is pending", async () => {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    ...
    snapshotRequests += 1
    return snapshotRequests === 3 ? pendingSnapshot.promise : snapshot(snapshotRequests > 3)
  })
  const { source } = await display()
  await act(async () => { source.emit("signature-draft") })
  ...
  await act(async () => { pendingSnapshot.resolve(snapshot(false)) })
...
it("reloads background on a background event and on reconnect", async () => {
  ...
  expect(backgroundRequests).toBe(3)
  vi.useFakeTimers()
  await act(async () => { source.onerror?.(); await vi.advanceTimersByTimeAsync(250) })
  const reconnected = ControlledEventSource.instances[1]
  if (reconnected === undefined) throw new Error("Expected reconnected EventSource")
  await act(async () => { reconnected.onopen?.(); await vi.advanceTimersByTimeAsync(0) })

...
export function RosterManagementPanel({
  ...
  const refresh = async () =>
    queryClient.invalidateQueries({ queryKey: rosterQueryKey(boardId) });
  const notifyFailure = (action: string, error: unknown) =>
    showToast(`${action}. ${describeRosterError(error)}`);
  const addition = useMutation({
    mutationFn: (identity: RosterIdentity) =>
      createRosterEntry(boardId, identity),
  ...
  const update = useMutation({
    mutationFn: ({
      ...
      readonly identity: RosterIdentity;
  ...
  const replacement = useMutation({
    mutationFn: (rows: readonly RosterIdentity[]) =>
      replaceRoster(boardId, rows),
    onError: (error) =>
      notifyFailure("서버 명단으로 갱신하지 못했습니다", error),
    onSuccess: (snapshot) => {
      queryClient.setQueryData(rosterQueryKey(boardId), snapshot);
  ...
  const fileImport = useMutation({
    mutationFn: ({
      ...
      readonly file: File;
      readonly preview: RosterPreview;
  ...
  const isSaving =
    ...
    update.isPending ||
    removal.isPending ||
    signatureRemoval.isPending ||
    replacement.isPending ||
    fileImport.isPending;
  const mutationError =
    addition.error ??
    update.error ??
    removal.error ??
    signatureRemoval.error ??
    replacement.error ??
    fileImport.error;
  const serverMarkers =
    fileImport.error instanceof RosterImportRejectedError
      ? fileImport.error.markers
      : [];

  return (
    <section
      className="editor-panel editor-roster-management"
      aria-labelledby="roster-management-title"
    >
      <h2 id="roster-management-title">명단 편집</h2>
      {mutationError !== null ? (
        <p aria-label="명단 유지 안내" role="alert">
          입력 내용을 반영하지 못했습니다. 표시된{" "}
          <span className="board-keep-together">서버 명단은</span> 그대로
          유지됩니다.
        </p>
      ) : null}

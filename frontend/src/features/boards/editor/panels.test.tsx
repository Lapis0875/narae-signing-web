...
describe("SharePanel", () => {
  it("Given clipboard rejection When copy is activated Then a visible generic failure is shown", async () => {
    ...
    render(<SharePanel disabled={false} onForceReplace={async () => undefined} onReissue={async () => undefined} share={{ shareToken: "safe-share", version: 1 }} />)
  ...
  it("Given an active display When replacement is cancelled or confirmed Then cancel sends none and confirm sends one request without changing the share URL", async () => {
    ...
    render(<SharePanel disabled={false} onForceReplace={replaceDisplay} onReissue={async () => undefined} share={{ shareToken: "safe-share", version: 1 }} />)
    ...
    expect(await screen.findByText(/현재 행사장 화면의 표시 연결을 종료/u)).toBeVisible()

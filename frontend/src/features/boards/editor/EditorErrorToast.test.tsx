...
it("explains an incompatible board response in the editor toast", async () => {
  ...
  expect(screen.getByRole("status")).toHaveTextContent("HTTP 200");
  expect(screen.getByRole("status")).toHaveTextContent("MALFORMED_RESPONSE");
  expect(screen.getByRole("status")).not.toHaveTextContent("status");
});
...
it("explains a rejected roster addition in the toast", async () => {
  ...
  render(
    <QueryClientProvider client={client}>
      <ToastProvider>
        <RosterManagementPanel boardId={boardId} entries={[]} />
      </ToastProvider>
    </QueryClientProvider>,
  );
  ...
  expect(screen.getByLabelText("이름")).toHaveValue("나래");
  expect(screen.getByText("서버 명단은")).toHaveClass("board-keep-together");
  expect(
    screen.getByRole("alert", { name: "명단 유지 안내" }),
  ).toHaveTextContent(
    "입력 내용을 반영하지 못했습니다. 표시된 서버 명단은 그대로 유지됩니다.",
  );
});

...
| board | UUID, owner_id, 평문 제목, 상태, 캔버스 폭·높이, 배경 참조, 공유 링크 버전·조회 해시·암호문·nonce·key_version, 생성·수정 시각 |
...
- 보호 대상: MinIO에 보관하는 배경 이미지 객체, PostgreSQL의 명단 원문과 서명 획, 현재 공유 링크 토큰.
- 평문 유지 대상: 보드 제목, 보드 상태, 소유자 참조, 캔버스·슬롯 좌표, 시간, 객체 키, 제출 여부, 키 버전, HMAC 식별값, 공유 링크 조회 해시와 버전.
...
- 링크 상태 조회·서명 화면 조회·SSE 같은 GET 읽기 요청은 CSRF 검사를 하지 않는다.
- 링크 상태 조회는 첫 identify 전에 읽을 수 있는 XSRF 토큰을 발급한다. identify와 서명 제출은 해당 쿠키 값과 `X-XSRF-TOKEN` 헤더가 일치해야 하며 Origin 검사는 추가 방어일 뿐 인증이나 CSRF 검사를 대신하지 않는다.
- 만료된 서명자 세션·무효 링크·마감 보드·속도 제한 응답은 원문 명단이나 내부 상태를 드러내지 않는 안정적인 오류 코드와 일반 안내만 사용한다. 속도 제한은 `Retry-After`를 함께 반환한다.
...
- 공유 링크 토큰은 32바이트 무작위 값을 패딩 없는 base64url로 표현한다. DB에는 공개 조회용 SHA-256 해시와 버전을 평문으로 두고, 현재 토큰은 소유자만 복구할 수 있도록 버전에 결합한 AES-GCM 암호문·nonce·key_version으로 저장한다.
...
- 보드 생명주기, 제목, 공유 링크 해시 조회·암호화된 현재 토큰 복구·재발급, 명단 직접 편집·일괄 반영을 구현한다.

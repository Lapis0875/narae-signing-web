...
실제 프로세스 시작 전 가드는 `psql`과 승인된 0600 권한의 `PGSERVICEFILE`을 사용한다. 가드는 연결 실패, 알 수 없는 대상, V5/V6가 섞인 카탈로그를 성공으로 취급하지 않는다. 다음 두 줄은 각각 새 릴리스와 이전 릴리스의 시작 래퍼 안에서, 해당 프로세스 시작 명령보다 먼저 실행해야 하는 필수 게이트다.
